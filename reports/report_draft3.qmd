...
# Create a long-form data set for the boxplot
injury_data_long <- cleaned_data %>%
  ...
  pivot_longer(cols = -BOROUGH, names_to = "Injury.Type", values_to = "Injuries") %>%
...
The boxplot of injuries in NYC by Borough shows the areas of NYC that have the most extreme number of injuries among pedestrians, cyclists, and motorists. The elevated numbers in certain boroughs are likely due to differences in borough population, amount of traffic, and the walkability and cyclability of the area. The most dangerous boroughs for pedestrians in regards to car crashes is Manhattan, Brooklyn for cyclists, and Queens for motorists. This graph gives us a better understanding of the relationship between borough and injuries.
