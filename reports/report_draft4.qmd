...
```{r echo=FALSE}
# Function to help with text mining for mapping contributing factors.
categorize_factors <- function(data_crashes) {
  categorized_data <- sqldf("
    SELECT *, 
    CASE 
      WHEN [CONTRIBUTING.FACTOR.VEHICLE.1] IN ('Driver Inattention/Distraction',
      'Alcohol Involvement', 'Aggressive Driving/Road Rage', 'Unsafe Speed',
      'Fell Asleep', 'Cell Phone (hand-held)', 'Texting', 'Driver Inexperience',
      'Lost Consciousness', 'Illnes', 'Fatigued/Drowsy',
      'Prescription Medication', 'Drugs (Illegal)', 'Illness') THEN
      'Driver Behavior'
      WHEN [CONTRIBUTING.FACTOR.VEHICLE.1] IN ('Brakes Defective',
      'Tire Failure/Inadequate', 'Headlights Defective', 'Tow Hitch Defective',
      'Vehicle Vandalism') THEN 'Vehicle Condition'
      WHEN [CONTRIBUTING.FACTOR.VEHICLE.1] IN ('Pavement Slippery',
      'View Obstructed/Limited', 'Obstruction/Debris', 'Glare',
      'Pavement Defective') THEN 'Road and Environmental Conditions'
      WHEN [CONTRIBUTING.FACTOR.VEHICLE.1] IN ('Traffic Control Disregarded',
      'Failure to Yield Right-of-Way', 'Passing or Lane Usage Improper',
      'Traffic Control Device Improper/Non-Working') THEN 'Traffic Control'
      ELSE 'Other'
...
bubble_data$Size <- cut(bubble_data$Count, breaks = pretty(bubble_data$Count, n = 10), include.lowest = TRUE, right = FALSE, labels = FALSE)

# Create the bubble chart with discrete sizes
ggplot(bubble_data, aes(x = Contributing_Factor_Category, y = VEHICLE_CATEGORY, size = factor(Size))) +
