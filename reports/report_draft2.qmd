...
numeroc_vars <- c("LATITUDE", "LONGITUDE", "CRASH.TIME")
model_data[numeroc_vars] <- lapply(model_data[numeroc_vars], function(x) (x - min(x)) / (max(x) - min(x)))
...
```
The decision tree model was trained to predict the number of people killed in car crashes based on the following features: BOROUGH, LATITUDE, LONGITUDE, CRASH.TIME, and PERSON_TYPE. The model was trained on 70% of the data and tested on the remaining 30%. The mean absolute error (MAE) of the model was calculated to be 0.00417116746503942. The decision tree model is visualized above. The model only has a maximum depth of 3, but it is a good starting point for predicting the number of people killed in car crashes. The model could be improved by using more features and more data.
