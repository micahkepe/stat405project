...
injury_data_long <- cleaned_data %>%
  select(BOROUGH, NUMBER.OF.PEDESTRIANS.INJURED, 
         NUMBER.OF.CYCLIST.INJURED, NUMBER.OF.MOTORIST.INJURED) %>%
  pivot_longer(cols = starts_with("NUMBER.OF"), 
               names_to = "Injury.Type", 
               values_to = "Injuries") %>%
  mutate(Injury.Type = factor(Injury.Type, 
...
plot <- ggplot(agg_data, aes(x = YEAR, y = COUNT, 
...
TODO: update this modeling section with new code and interpretation of the results.
