...
# Perform data cleaning and pre-processing
cleaned_data <- data
cleaned_data <- cleaned_data[!is.na(cleaned_data$BOROUGH) & cleaned_data$BOROUGH != "", ]
cleaned_data <- cleaned_data[!is.na(cleaned_data$LATITUDE) & cleaned_data$LATITUDE > 40 & cleaned_data$LATITUDE < 41, ]
cleaned_data <- cleaned_data[!is.na(cleaned_data$LONGITUDE) & cleaned_data$LONGITUDE < -73 & cleaned_data$LONGITUDE > -75, ]
...
cleaned_data$CRASH.TIME <- format(strptime(cleaned_data$CRASH.TIME, format = "%H:%M"), "%H:%M:%S")

# Replace empty strings with "UNKNOWN" for street names
street_name_columns <- c("ON.STREET.NAME", "CROSS.STREET.NAME", "OFF.STREET.NAME")
cleaned_data[street_name_columns] <- lapply(cleaned_data[street_name_columns], function(x) ifelse(x == "", "UNKNOWN", x))
...
ggplot(cleaned_data, aes(x = LONGITUDE, y = LATITUDE)) +
  ...
  theme_minimal() +
  geom_text(data = landmarks, aes(x = longitude, y = latitude, label = name), vjust = -1, check_overlap = TRUE, color = "darkblue", size = 3)
...
factors <- cleaned_data[cleaned_data$CONTRIBUTING.FACTOR.VEHICLE.1 %in% c("Driver Inattention/Distraction",
...
# Create the plot
ggplot(data = factors, aes(x = CONTRIBUTING.FACTOR.VEHICLE.1, y = NUMBER.OF.PERSONS.INJURED)) +
...
The point plot shows the number of injuries per contributing factor, illustrating that driver inattention/distraction result in more frequent instances of high injury crashes. This graph gives us a better understanding of the relationship between contributing factor and injury outcomes.
...
# Group by hour and contributing factor, then count occurrences
factor_by_hour <- top_factors %>%
  group_by(HOUR, CONTRIBUTING.FACTOR.VEHICLE.1) %>%
  summarise(Count = n())

# Scatter plot of contributing factors by hour of day
...
```{r echo=FALSE}
# Assuming 'cleaned_data' is your data frame and already loaded
...
plot_data <- subset(plot_data, CONTRIBUTING.FACTOR.VEHICLE.1 != "Unspecified" & CONTRIBUTING.FACTOR.VEHICLE.1 != "")

# Aggregate data by YEAR and CONTRIBUTING FACTOR
agg_data <- aggregate(cbind(COUNT = plot_data$COLLISION_ID) ~ YEAR + CONTRIBUTING.FACTOR.VEHICLE.1, data=plot_data, FUN=length)

# Find the top 5 contributing factors based on the overall count
top_factors <- aggregate(COUNT ~ CONTRIBUTING.FACTOR.VEHICLE.1, data=agg_data, FUN=sum)
...
plot <- ggplot(agg_data, aes(x = YEAR, y = COUNT, fill = CONTRIBUTING.FACTOR.VEHICLE.1)) +
...
```{r echo=FALSE}
ggplot(data = cleaned_data[cleaned_data$NUMBER.OF.PERSONS.INJURED + cleaned_data$NUMBER.OF.PERSONS.KILLED > 0
                           & cleaned_data$CONTRIBUTING.FACTOR.VEHICLE.1 %in% c("Driver Inattention/Distraction",
                                                                                "Failure to Yield Right-of-Way",
                                                                                "Following Too Closely",
                                                                                "Other Vehicular",
                                                                                "Passing Too Closely"),]) +
...
# ATTEMPTED CODE FOR NEURAL NETWORK MODEL (TODO: FIX AND UPDATE)
# # Further data cleaning and pre-processing for the neural network model
# nn_data <- cleaned_data %>%
#   select(NUMBER.OF.PERSONS.INJURED, CRASH.TIME, BOROUGH, CONTRIBUTING.FACTOR.VEHICLE.1) %>%
#   mutate(
#     HOUR = as.numeric(substr(CRASH.TIME, 1, 2)),
#     BOROUGH = as.factor(BOROUGH),
#     CONTRIBUTING.FACTOR.VEHICLE.1 = as.factor(CONTRIBUTING.FACTOR.VEHICLE.1)
#   ) %>%
...
# nn_data_transformed <- model.matrix(~ HOUR + BOROUGH + CONTRIBUTING.FACTOR.VEHICLE.1 - 1, data = nn_data)
# 
# nn_target <- nn_data$NUMBER.OF.PERSONS.INJURED
# 
# # Now, when you sample indices for training, ensure both x_train/x_test and y_train/y_test are correctly assigned
# train_indices <- sample(1:nrow(nn_data_transformed), 0.8 * nrow(nn_data_transformed))
# x_train <- nn_data_transformed[train_indices, ]
# y_train <- nn_target[train_indices]  # Ensure this is capturing the correct subset
# x_test <- nn_data_transformed[-train_indices, ]
# y_test <- nn_target[-train_indices]  # Ditto for y_test
# 
# # Verify the dimensions and presence of the target data
# print(dim(x_train))
# print(length(y_train))  # Use length for a 1D vector like y_train
# 
...
#   layer_dense(units = 1, input_shape = c(num_features)) # simple linear regression model
# 
# # Compile the model
# model %>% compile(
#   optimizer = "adam",
...
TODO: update this conclusion with new findings from the updated data analysis and modeling.
