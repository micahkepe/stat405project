...
analysis_data$DayOfWeek <- weekdays(analysis_data$CRASH.DATE)
...
<!-- model_data <- merged_data[, c("BOROUGH", "LATITUDE", "LONGITUDE",  -->
<!--                               "CONTRIBUTING.FACTOR.VEHICLE.1",  -->
...
<!-- # Abbreviate the contributing factor column for easier reading -->
<!-- # Create named vector of abbreviated labels -->
<!-- factor_abbreviations <- setNames(c("Driver Inattention/Distraction" = "Inattention", -->
<!--                                    "Failure to Yield Right-of-Way" = "Failure to Yield", -->
<!--                                    "Backing Unsafely" = "Backing", -->
<!--                                    "Following Too Closely" = "Following Closely", -->
...
<!-- # Define a function to abbreviate the contributing factor labels -->
<!-- abbreviate_factors <- function(x, abbreviations) { -->
<!--   sapply(x, function(value) { -->
<!--     if (value %in% names(abbreviations)) { -->
<!--       return(abbreviations[value]) -->
<!--     } else { -->
<!--       return(abbreviate(value, minlength = 20))  # Fall back to automatic abbreviation -->
<!--     } -->
<!--   }) -->
<!-- } -->

<!-- model_data$CONTRIBUTING.FACTOR.VEHICLE.1.ABBR <- abbreviate_factors(model_data$CONTRIBUTING.FACTOR.VEHICLE.1, factor_abbreviations) -->

<!-- # Convert categorical variables to factors -->
<!-- model_data$BOROUGH <- as.factor(model_data$BOROUGH) -->
<!-- model_data$PERSON_TYPE <- as.factor(model_data$PERSON_TYPE) -->
<!-- model_data$CONTRIBUTING.FACTOR.VEHICLE.1.ABBR <- as.factor(model_data$CONTRIBUTING.FACTOR.VEHICLE.1.ABBR) -->
...
<!-- tree_model <- rpart(NUMBER.OF.PERSONS.KILLED ~ BOROUGH + LATITUDE + LONGITUDE + CONTRIBUTING.FACTOR.VEHICLE.1.ABBR + PERSON_TYPE,  -->
<!--                     data = train_data,  -->
<!--                     method = "anova", -->
<!--                     control = rpart.control(maxdepth = 3 , minsplit = 10, cp = 0.001)) -->
